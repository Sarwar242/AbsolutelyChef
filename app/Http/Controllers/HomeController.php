...
class HomeController extends Controller
{
    ...
    public function index(){
        ...
        $jobsCount = Job::active()->count();
        $employersCount = User::where('user_type','employer')->count();
        $blog_posts = Post::whereType('post')->with('author')->orderBy('id', 'desc')->take(3)->get();
        $packages = Pricing::all();
        return view('home', compact('categories', 'premium_jobs','regular_jobs','jobsCount','employersCount','packages', 'blog_posts'));
    }
    ...
    public function advertise(){
        ...
        $title = __('app.advertise');
        $p_waitings = Package::where('type',PackageTypeEnums::ENTERPRISE)->get();
        $p_profational = Package::where('type',PackageTypeEnums::PROFESSIONAL)->get();
        $waitings = Package::all();
        return view('advertise', compact('title','p_waitings','p_profational'));
    }

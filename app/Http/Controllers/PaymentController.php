...
use App\Notifications\PremiumInvoiceMail;
use Illuminate\Support\Facades\Log;

class PaymentController extends Controller
{
    public function paypremium(Request $request, $nonce, $idempotency_key, $token, $general, $management, $total_pay)
    {   
        $user = auth()->user();
        ...
        // optional
        $note = 'Payment for Premium'; //note about this charge

        //optional
        $reference_id = $general+$management; //some kind of reference id to an object or resource
        ...
        if($payment_status==OrderPaymentStatusEnum::SUCCESS)
        {
            ...
            $payment = $user->payments()->create([
                ...
                'jobs'    =>  $reference_id,
                'package_name'    => 'Premium Jobs',
            ]);
            $user->premium_jobs_general= $user->premium_jobs_general+(int)$general;
            $user->premium_jobs_management=$user->premium_jobs_management+(int)$management;
            $user->save();
            try{
                $user->notify(new PremiumInvoiceMail($payment));
            }catch(\Exception $e){
                Log::info('Mail couldn\'t be sent. '.$e->getMessage());
            }    
            return view('admin.paymentsuccesspremium', compact(['user','payment']));
        }
        else{
    ...
    public function markSuccess($id, $status){
        ...
        $payment->save();
        $trans= DB::update('update nikolag_transactions set status ="PAID" where payment_service_id ='.$payment->transaction_id);
        if ($status === 'success'){
